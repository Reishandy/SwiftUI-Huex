...
/// Reads RGB values from every pixel in `image`. Sizing/downsampling is the
/// caller's responsibility (PhotoDataWorker already requests a small target
/// size from PHImageManager before calling into this pipeline) — this stays
/// a plain pixel reader, not a resizer.
///
...
nonisolated func extractRGBVectors(from image: UIImage) -> [simd_float3]? {
...
nonisolated func categorize(
	swatches: [Swatch],
	minCoveragePercentage: Double = 0.08,
	chromaTieThreshold: Double = 5.0,
	marginThreshold: Double = 0.10
	...
	let ranked = eligible.sorted { a, b in
		let chromaDiff = abs(a.lch.c - b.lch.c)
		if chromaDiff < chromaTieThreshold {
	...
	if ranked.count > 1 {
		...
		let denominator = max(primary.lch.c, 1)
		margin = (primary.lch.c - runnerUp.lch.c) / denominator
	} else {
		margin = 1.0
	}
	...
	return CategorizationResult(bucket: bucketFor(swatch: primary), confidence: primary.weight, margin: margin)
}

/// LCh gates evaluated in order — lightness extremes, then chroma floor,
/// then pink/brown windows, then nearest hue by circular distance. Hue is
/// only ever consulted last, since it's unstable/meaningless at low chroma
/// or extreme lightness.
nonisolated func bucketFor(
	...
	// Pink — a lightness/chroma modifier on red, not a separate hue.
	if (h >= 330 || h <= 20) && l > 55 && c < 45 {
		return .pink
	}
	
	// Brown — a lightness/chroma modifier on orange/yellow, not a separate hue.
	if h >= 20 && h <= 70 && l < 65 && c < 45 {
		return .brown
	}
...
// MARK: - Top-level entry point (called from PhotoDataWorker)
